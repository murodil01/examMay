    ...
    <div className="max-w-[1200px] mx-auto mt-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((product) => (
          <div key={product.id} className="border relative shadow-sm hover:shadow-md transition duration-300 p-3 rounded">
            <button onClick={() => toggleLike(product.id)} className="absolute top-3 right-3 z-10">
              <Heart size={22} className={liked[product.id] ? "text-red-500 fill-red-500" : "text-gray-400"} />
            </button>
            ...
            <img src={product.img} alt={product.name} className="w-full h-[300px] object-cover rounded" />
