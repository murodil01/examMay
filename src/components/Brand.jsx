...
const Brand = ({ cart, setCart, likedItems, setLikedItems, toggleLike }) => {
...
const Brand = ({ cart, setCart, likedItems, setLikedItems }) => {
  ...
  const toggleCartModal = () => setModalVisible((prev) => !prev);
  const toggleLikedModal = () => setLikedVisible((prev) => !prev);
        ...
        {likedItems.length === 0 ? (
          ...
          likedItems.map((item) => (
            ...
            >
              ...
              <div>
                <h4 className="font-semibold">{item.name}</h4>
                <p className="text-red-500">{item.discountPrice || item.price} sum</p>
              </div>
            </div>
          ))
        )}
